...
export default function IdeaDetailsPage() {
  ...
  if (!idea) {
    return (
      <LayoutWrapper>
        <div className="bg-slate-950 border border-slate-800 rounded-3xl p-12 text-center max-w-lg mx-auto mt-12 space-y-4">
          <div className="p-4 bg-slate-900 w-16 h-16 rounded-2xl flex items-center justify-center border border-slate-800 text-rose-500 mx-auto">
        ...
        <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4 border-b border-slate-800/80 pb-6">
          <div className="space-y-2">
            ...
            <div className="flex items-center gap-3.5 flex-wrap">
              <h1 className="text-3xl font-extrabold text-white tracking-tight">{idea.title}</h1>
          ...
          {/* Voting Box */}
          <div className="flex items-center gap-3.5 bg-slate-950 border border-slate-800 p-3 rounded-2xl self-start lg:self-auto">
            <span className="text-xs text-slate-400 font-semibold px-2">Vote on Draft</span>
            <div className="flex items-center gap-1">
              ...
              <span className="text-sm font-bold text-slate-200 px-1.5 min-w-[20px] text-center">
                {voteScore}
              </span>
              <button
                onClick={() => handleVote(-1)}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-8">
            ...
            <div className="bg-slate-950 border border-slate-800 p-6 rounded-2xl space-y-4">
              <h3 className="text-sm font-bold text-white uppercase tracking-wider">Proposal Overview</h3>
              ...
              {idea.expectedBenefits && (
                <div className="pt-4 border-t border-slate-900 space-y-2">
                  <h4 className="text-xs font-bold text-indigo-400 uppercase tracking-wider">Expected Benefits</h4>
            ...
            {/* AI Executive Summary Sandbox Section */}
            <div className="bg-slate-950 border border-slate-800 p-6 rounded-2xl space-y-4">
              <div className="flex items-center justify-between">
                ...
                {!summary && (
                  <button
                    ...
                    className="bg-indigo-600/10 hover:bg-indigo-600/20 text-indigo-400 text-xs font-semibold px-3 py-1.5 rounded-xl border border-indigo-500/20 transition-all flex items-center gap-1"
                  >
              ...
              {isSummarizing ? (
                <div className="p-8 text-center bg-slate-900/40 rounded-xl border border-slate-800 space-y-3 animate-pulse">
                  <div className="w-8 h-8 rounded-full border-2 border-slate-800 border-t-indigo-500 animate-spin mx-auto" />
                  <p className="text-xs text-slate-500 font-mono">Consolidating evaluation data & compiling brief...</p>
                </div>
              ) : summary ? (
                <div className="bg-gradient-to-br from-indigo-500/5 to-violet-500/5 p-5 rounded-xl border border-indigo-500/10 text-slate-300 text-xs leading-relaxed space-y-3 shadow-inner">
            ...
            <div className="space-y-4">
              <h3 className="text-sm font-bold text-white uppercase tracking-wider">AI Domain-Agent Evaluations</h3>
              
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                {reviews.map((review) => (
                  <div
                    key={review.id}
                    className="bg-slate-950 border border-slate-800/80 p-5 rounded-2xl flex flex-col justify-between shadow-sm relative overflow-hidden group hover:border-slate-700 transition-all"
                  >
                    <div className="space-y-3">
                      <div className="flex items-center justify-between">
                        ...
                        <span className={`text-xs font-black border rounded px-2 py-0.5 ${getScoreColor(review.score)}`}>
                          {review.score}/100
                        </span>
                      </div>
                    ...
                    <div className="border-t border-slate-900/60 pt-3 mt-4 flex items-center justify-between text-[10px] text-slate-500 font-semibold">
                      ...
                      <span>Verified Sandbox Protocol</span>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <div className="bg-slate-950 border border-slate-800 p-6 rounded-2xl space-y-6">
              <h3 className="text-sm font-bold text-white uppercase tracking-wider">Discussion Board</h3>
              ...
              {currentUser && (
                <form onSubmit={handleAddComment} className="flex gap-3">
                  <input
                    ...
                    className="flex-1 bg-slate-900 border border-slate-800 rounded-xl px-4 py-3 text-xs text-slate-100 placeholder:text-slate-500 focus:outline-none focus:border-indigo-500 transition-all"
                  />
                  <button
                    type="submit"
              ...
              <div className="space-y-4 pt-2">
                {comments.length === 0 ? (
                  ...
                  comments.map((comment) => (
                    <div key={comment.id} className="bg-slate-900/60 border border-slate-800/60 p-4 rounded-xl space-y-1.5">
                      <div className="flex items-center justify-between text-[10px] font-semibold">
                        <span className="text-indigo-400">User #{comment.userId.split('_')[1] || comment.userId}</span>
          ...
          <div className="space-y-6">
            ...
            <div className="bg-slate-950 border border-slate-800 p-6 rounded-2xl text-center space-y-4">
              ...
              <div className="relative w-36 h-36 mx-auto flex items-center justify-center">
                ...
                <div className="text-center">
                  <span className="text-4xl font-black text-white">{idea.innovationScore}</span>
                  <span className="text-slate-500 text-xs block mt-0.5">out of 100</span>
                </div>
              </div>

              <div className="space-y-1.5 text-xs text-slate-400">
                <p className="font-semibold text-slate-300">Agent Aggregate consensus reached</p>
            ...
            {/* Role-Based Manager Approval & Reject Sandbox Panel */}
            {(currentUser?.role === 'manager' || currentUser?.role === 'admin') && (
              <div className="bg-slate-950 border border-amber-500/20 bg-amber-500/[0.01] p-6 rounded-2xl space-y-4">
                ...
                <p className="text-xs text-slate-400 leading-relaxed">
                  As a logged-in <span className="font-bold text-slate-300">{currentUser.role.toUpperCase()}</span>, you have sandbox authority to update the status of this idea in real-time.
                </p>
                ...
                <div className="grid grid-cols-1 gap-2.5 pt-2">
                  ...
                  {idea.status !== 'under_review' && (
                    <button
                      ...
                      className="w-full bg-slate-900 border border-slate-800 hover:bg-slate-800 text-slate-300 font-semibold text-xs px-4 py-2.5 rounded-xl transition-all"
                    >
